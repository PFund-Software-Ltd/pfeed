...
class DuckDBIO(DatabaseIO, FileIO):
    ...
    def write(self, data: pd.DataFrame | pl.LazyFrame | pa.Table, db_path: DBPath, delete_where: str | None = None):
        ...
        null_cols = [
            name for name, dtype in nw.from_native(data).collect_schema().items()
            if dtype == nw.Unknown
        ]
