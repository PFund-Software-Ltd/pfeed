...
PFEED_PARQUET_FILE_RE = re.compile(r'^(?P<symbol>.+)_(?P<date>\d{4}-\d{2}-\d{2})\.parquet$')
HIVE_PART_RE = re.compile(r'^([^=]+)=(.*)$')
DUCKDB_LOCK_RE = re.compile(r'Conflicting lock is held in (\S+).*?\(PID (\d+)\)', re.DOTALL)

HIVE_ORDER = ["env", "data_layer", "data_domain", "data_source", "data_origin", "asset_type", "resolution"]

# Columns that may auto-hide from `ls` when every visible row carries the same value.
COLLAPSIBLE_COLUMNS = ("storage", "env", "data_layer", "data_domain",
                       "data_source", "asset_type", "resolution", "backend")
COLUMN_LABELS = {
    "storage": "Storage", "env": "Env", "data_layer": "Layer", "data_domain": "Domain",
    "data_source": "Source", "asset_type": "AssetType", "resolution": "Resolution",
    "backend": "Backend",
}
...
class DataUnit:
    ...
    def symbol_cell(self) -> str:
        ...
        if not self.symbols:
            return "—"
        return self.symbols[0] if len(self.symbols) == 1 else f"<{len(self.symbols)} products>"
...
def _walk_hive(root: Path) -> Iterator[tuple[dict[str, str], Path]]:
    ...
    def rec(d: Path, depth: int, ctx: dict[str, str]) -> Iterator[tuple[dict[str, str], Path]]:
...
def _scan_delta(table_dir: Path) -> tuple[list[datetime.date], list[str], int, datetime.datetime | None]:
        ...
        for daydir in table_dir.rglob("day=*"):
            try:
                ...
                if mday and mmon and myr:
                    dates.add(datetime.date(int(myr.group(2)), int(mmon.group(2)), int(mday.group(2))))
            except Exception:
                pass
...
def _discover_file_based(storage_name: str, data_path: Path) -> Iterator[DataUnit]:
    for ctx, leaf in _walk_hive(data_path):
        ...
        if has_delta:
            ...
            yield DataUnit(
                storage=storage_name, backend="deltalake",
                env=ctx.get("env", ""), data_layer=ctx.get("data_layer", ""),
                data_domain=ctx.get("data_domain", ""), data_source=ctx.get("data_source", ""),
                data_origin=ctx.get("data_origin", ""), asset_type=ctx.get("asset_type", ""),
                resolution=ctx.get("resolution", ""),
                symbols=symbols, dates=dates, size_bytes=size,
                last_modified=mtime, path=str(leaf),
            )
        ...
        for sym, info in sorted(by_symbol.items()):
            mtime = datetime.datetime.fromtimestamp(info["mtime"]) if info["mtime"] else None
            yield DataUnit(
                storage=storage_name, backend="parquet",
                env=ctx.get("env", ""), data_layer=ctx.get("data_layer", ""),
                data_domain=ctx.get("data_domain", ""), data_source=ctx.get("data_source", ""),
                data_origin=ctx.get("data_origin", ""), asset_type=ctx.get("asset_type", ""),
                resolution=ctx.get("resolution", ""),
                symbols=[sym], dates=sorted(info["dates"]), size_bytes=info["size"],
                last_modified=mtime, path=str(leaf),
            )
...
def _discover_duckdb(roots: list[tuple[str, Path]], errors: list[str] | None = None) -> Iterator[DataUnit]:
...
def _process_duckdb_file(db_file: Path, root: Path, storage_label: str,
                          errors: list[str] | None) -> Iterator[DataUnit]:
    ...
    try:
        ...
        yield DataUnit(
            storage=storage_label, backend="duckdb",
    ...
    try:
        ...
        for (schema_name,) in schemas:
            ...
            for table_name in non_meta:
                ...
                yield DataUnit(
                    storage=storage_label, backend="duckdb",
                    env=env, data_layer=layer, data_domain=domain,
                    data_source=source, data_origin=origin,
                    asset_type=asset_type, resolution=resolution,
...
def _hierarchical_sort_key(u: DataUnit) -> tuple:
    return (u.storage, u.data_layer, u.env, u.data_source, u.data_origin,
            u.asset_type, u.resolution, u.symbol_cell)
...
def _build_overview_table(units: list[DataUnit]):
    ...
    for u in units:
        groups.setdefault((u.storage, u.backend, u.data_layer, u.data_domain), []).append(u)
    ...
    for key, group in sorted(groups.items()):
        ...
        t.add_row(
            storage, backend, layer or "—", domain or "—",
            f"{len(group):,}",
...
def _build_ls_table(units: list[DataUnit]):
    ...
    if shared:
        labels = " · ".join(f"{COLUMN_LABELS[a]}={shared[a]}" for a in COLLAPSIBLE_COLUMNS if a in shared)
...
def _filter_options(f):
    f = click.option("--resolution", "-r", default=None, help="Filter by resolution (e.g. 1_MINUTE)")(f)
    f = click.option("--symbol", default=None, help="Filter by product symbol")(f)
    f = click.option("--asset-type", "asset_type", default=None, help="Filter by asset type")(f)
    f = click.option("--env", default=None, help="Filter by env (BACKTEST/PAPER/LIVE)")(f)
    f = click.option("--source", default=None, help="Filter by data source (e.g. BYBIT)")(f)
    f = click.option("--domain", "-d", default=None, help="Filter by data domain (e.g. MARKET_DATA)")(f)
    f = click.option("--layer", "-l", default=None, help="Filter by data layer (RAW/CLEANED/CURATED)")(f)
    f = click.option("--storage", "-s", default=None, help="Filter by storage (LOCAL/CACHE/DUCKDB)")(f)
    return f


def _collect(storage, layer, domain, source, env, asset_type, symbol, resolution,
             errors: list[str] | None = None):
    units = _gather_all_units(errors=errors)
    return _apply_filters(
        units,
        storage=storage, data_layer=layer, data_domain=domain,
        data_source=source, env=env, asset_type=asset_type,
...
@click.option("--all", "show_all", is_flag=True, help="Show everything (ignore --limit)")
@click.option(
    "--sort", "sort_by", default="hierarchy", show_default=True,
    type=click.Choice(["hierarchy", "source", "symbol", "dates", "size", "updated"]),
)
@click.option("--json", "as_json", is_flag=True, help="Emit JSON instead of a table")
def list_data(storage, layer, domain, source, env, asset_type, symbol, resolution,
              limit, show_all, sort_by, as_json):
    """List datasets/tables (dates collapsed). Columns shared across all rows auto-hide."""
    errors: list[str] = []
    units = _collect(storage, layer, domain, source, env, asset_type, symbol, resolution, errors=errors)
    sorters = {
...
def tree(storage: str | None):
    ...
    HIVE_BRANCH_LEVELS = ("env", "data_layer", "data_domain", "data_source",
                          "data_origin", "asset_type", "resolution")
    ...
    for storage_label in sorted(by_storage.keys()):
        ...
        hive_units = [u for u in storage_units if u.backend not in {"duckdb", "lancedb"}]
        ...
        for db_path, db_us in sorted(by_db_file.items()):
            ...
            db_node = storage_node.add(f"[magenta]{display}[/magenta]  [dim]({db_us[0].backend})[/dim]")
            for u in db_us:
                if u.status:
                    db_node.add(
        ...
        if hive_units:
            ...
            def walk(d: dict[Any, Any], parent):
                for k in sorted(kk for kk in d.keys() if kk != "__leaves__"):
