...
from typing import Callable, TYPE_CHECKING, Any, ClassVar, cast, ParamSpec, Concatenate
if TYPE_CHECKING:
    from collections.abc import Awaitable
    from pfeed.sources.base_source import BaseSource
    from pfeed.storages.base_storage import BaseStorage
    from pfeed.data_models.base_data_model import BaseDataModel
    from pfeed.feeds.streaming_feed_mixin import ChannelKey, WebSocketName, RawMessage, ReplayData
...
from pfeed.enums import ExtractType, DataSource
...
class Faucet:
    ...
    def __init__(
        ...
        extract_func: Callable[Concatenate[BaseDataModel, ExtractFuncParams], Any],  # e.g. _download_impl(), _stream_impl(), _retrieve_impl(), _fetch_impl()
        ...
        self._streaming_queue: asyncio.Queue[tuple[WebSocketName | DataSource, RawMessage | ReplayData] | None] | None = None
        self._user_callback: Callable[[WebSocketName | DataSource, RawMessage | ReplayData], Awaitable[None] | None] | None = None
        self._streaming_bindings: dict[ChannelKey, DataFlow] = {}
        self._msg_queue: ZeroMQ | None = None
        self._stream_workers: list[str] = []

    @property
    def streaming_queue(self) -> asyncio.Queue[tuple[WebSocketName | DataSource, RawMessage | ReplayData] | None]:
    ...
    async def open_stream(self, data_model: BaseDataModel, storage: BaseStorage | None = None):
        ...
        if not self._is_stream_opened:
            ...
            await self._extract_func(data_model=data_model, faucet_callback=self._callback, storage=storage)
    ...
    async def _callback(
        ...
        if self._user_callback:
            result = self._user_callback(source, data)
        ...
        if self._streaming_queue:
            if self._streaming_queue.full():
                self._logger.warning(
                    "Streaming queue full, dropping oldest message - consider increasing maxsize " +
                    f"(current: {self.STREAMING_QUEUE_MAXSIZE}) or improving consumer speed"
                )
                _ = self._streaming_queue.get_nowait()  # Remove oldest
            await self._streaming_queue.put((source, data))
        # dispatch to bound dataflow
        if channel_key:
        ...
        # channel_key is None only for non-data control messages from a live websocket
        # (subscription acks, pongs). Never happens in replay.
    ...
    def set_user_callback(self, callback: Callable[[WebSocketName | DataSource, RawMessage | ReplayData], Awaitable[None] | None]):
        if self._user_callback is not None and self._user_callback != callback:
            from pfeed.utils import is_lambda
            msg = (
                f'streaming callback is already set, existing callback function: {self._user_callback}\n' +
                f'new callback function: {callback}'
            )
            if is_lambda(callback) or is_lambda(self._user_callback):
                msg += (
