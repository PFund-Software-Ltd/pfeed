    ...
    if isinstance(rollback_period, str) and rollback_period.lower() == "ytd":
        ...
        if rollback_period.is_day():
            ...
            if utcnow.month == 2 and utcnow.day == 29 and not calendar.isleap(year):
                day = 28
            target_date = datetime.datetime(year, utcnow.month, day, tzinfo=datetime.timezone.utc)
            timedelta = utcnow - target_date
        else:
...
def parse_date_range(
    ...
    if start_date:
        ...
        assert rollback_period, "rollback_period must be provided if start_date is not provided"
