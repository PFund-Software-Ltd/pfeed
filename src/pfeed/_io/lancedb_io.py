...
class LanceDBIO(DatabaseIO, TableIO):
        ...
        **io_kwargs: Any,
        ...
        if not self.exists(db_path):
            table = conn.create_table(
                db_path.table_name, data, schema=schema, mode="overwrite", **io_kwargs
            )
        else:
            table = self.get_table(db_path)
    ...
    def read(
        self, db_path: DBPath, where: str | None = None, **io_kwargs: Any
    ) -> pl.LazyFrame | None:
        ...
        if self.exists(db_path):
            ...
            result = (
                table.search().where(where).to_polars() if where else table.to_polars()
            )
