...
@click.option(
    "--resolution",
    "-r",
    help='Data resolution (e.g. "1m" for 1 minute data) or Data type (e.g. "tick"). If not provided, the lowest resolution of the data source will be used',
)
@click.option(
    "--rollback-period", "--rb", help='Rollback period (e.g. "1w" for 1 week of data)'
)
@click.option(
    "--start-date",
    "-s",
...
@click.option(
    ...
    "--layer",
    default="CLEANED",
    type=click.Choice([level.name for level in DataLayer], case_sensitive=False),
...
@click.option(
    ...
    "--destination",
    default="LOCAL",
    type=click.Choice(DataStorage, case_sensitive=False),
