...
class TimeBasedDataHandler(BaseDataHandler, ABC):
    ...
    def _partition_stream_data(table: pa.Table) -> pa.Table:
        ...
        return (
            table.append_column("year", pc.year(ts))
    ...
    def find_missing_dates_in_storage(self):
        existing_metadata_dict = cast(
            "dict[SourcePath, TimeBasedDataMetadata]", self.read_metadata()
        )
        existing_dates = []
        match self._io_type:
            case IOType.FILE:
    ...
    def _get_date_col(self) -> str:
        ...
        if self._data_layer == DataLayer.RAW:
            ...
            is_streaming_data = hasattr(
                self._data_model, "env"
            ) and self._data_model.env in (Environment.LIVE, Environment.PAPER)  # pyright: ignore[reportAttributeAccessIssue]
            ...
            case (IOType.TABLE | IOType.DATABASE) as io_type:
                ...
                if not df.is_empty():
                    ...
                    if not self.io.DATE_FILTER_PREDICATE:
                        raise ValueError(
                            f"IO {self.io.name} has no DATE_FILTER_PREDICATE"
                        )
                    delete_where = self.io.DATE_FILTER_PREDICATE.format(
                        date_col=date_col, start_date=start_date, end_date=end_date
                    )
                else:
                    delete_where = None
                ...
                self.io.write_metadata(
                    source_path, metadata=self._create_metadata(dates=data_model.dates)
                )
            case _:
                assert_never(self._io_type)
    ...
    def read(self) -> pl.LazyFrame | None:
        ...
        match self._io_type:
            ...
            case (IOType.TABLE | IOType.DATABASE) as io_type:
                ...
                start_date, end_date = (
                    self._data_model.start_date,
                    self._data_model.end_date,
                )
                if lf is not None:
                    # streaming 'ts' is int64 ns epoch; batch 'date' is a temporal type
                    col = pl.col(date_col)
                    col_as_date = (
                        pl.from_epoch(col, time_unit="ns").dt.date()
                        if date_col == "ts"
                        else col.dt.date()
                    )
                    lf = lf.filter(
                        (col_as_date >= start_date) & (col_as_date <= end_date)
                    )
            case _:
                assert_never(self._io_type)
        return lf
