...
from typing import TYPE_CHECKING, Any, ClassVar, Concatenate, ParamSpec, cast
...
class Faucet:
    ...
    def __init__(
        ...
        extract_func: Callable[
            Concatenate[BaseDataModel, ExtractFuncParams], Any
        ],  # e.g. _download_impl(), _stream_impl(), _retrieve_impl(), _fetch_impl()
        extract_type: ExtractType,
    ):
        self.data_source = data_source
        self._logger = logging.getLogger(f"pfeed.{self.data_source.name.lower()}")
        self.extract_type = (
            ExtractType[extract_type.lower()]
    ...
    async def _callback(
        ...
        if channel_key:
            if channel_key in self._streaming_bindings:
                ...
                self._logger.error(
                    f"Missing dataflow binding for channel key {channel_key}"
                )
    ...
    def bind_channel_key_to_dataflow(self, channel_key: ChannelKey, dataflow: DataFlow):
        if channel_key in self._streaming_bindings:
            raise ValueError(
                f"channel key {channel_key} is already bound to a dataflow"
            )
