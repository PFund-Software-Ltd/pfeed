...
class NewsFeed(TimeBasedFeed, ABC):
    ...
    def create_data_model(
        ...
        **product_specs: Any,
        ...
        if len(self.dataflows) > 0:
            ...
            if existing_env != env:
                raise ValueError(
                    f"{self.name} dataflows have different environments: {existing_env} and {env}"
                )
    ...
    def _create_data_model_from_request(
        self, request: NewsFeedBaseRequest
    ) -> NewsDataModel: ...
    ...
    def download(
        ...
        **product_specs: Any,
        ...
        if not product:
            assert not symbol, (
                "symbol should not be provided if product is not provided"
            )
            product = None
        else:
        ...
        start_date, end_date = self._standardize_dates(
            start_date, end_date, rollback_period
        )
        data_layer = DataLayer[data_layer.upper()]
        return self._run_download(
            partial_dataflow_data_model=partial(
                self.create_data_model,
                env=env,
                product=product,
                data_origin=data_origin,
            ),
            partial_faucet_data_model=partial(
                self.create_data_model,
                env=env,
                product=product,
                data_origin=data_origin,
            ...
            add_default_transformations=lambda: (
                self._add_default_transformations_to_download(
                    data_layer, product=product
                )
            ),
            load_to_storage=(lambda: self.load(to_storage, data_layer))
            if to_storage
            else None,
        )
    ...
    def _get_default_transformations_for_download(
        self, data_layer: DataLayer, product: BaseProduct | None = None
    ) -> list[Callable]:
    ...
    def retrieve(
        ...
        **product_specs: Any,
        ...
        product: BaseProduct | None = (
            self.data_source.create_product(product, **product_specs)
            if product
            else None
        )
        start_date, end_date = self._standardize_dates(
