...
class BatchAPI:
    ...
    DATA_NAMING_REGEX_PATTERNS: ClassVar[dict[str, str]] = {
        ...
        AssetTypeModifier.INVERSE
        + "-"
        + CryptoAssetType.PERPETUAL: r"USD\/$",  # inverse perps;
        AssetTypeModifier.INVERSE
        + "-"
        + CryptoAssetType.FUTURE: r"USD[A-Z]\d{2}\/$",  # inverse futures e.g. BTCUSDH24/
    ...
    def _create_filename(
        product: BybitProduct, resolution: Resolution, date: str | datetime.date
    ) -> str:
        if resolution.is_quote():
    ...
    def _get(url: str, params: dict[str, Any] | None = None) -> bytes | None:
        ...
        while num_retries:
            ...
            try:
                ...
                cprint(
                    f"Error: failed to get data from {url}, {exc=}",
                    style=TextStyle.BOLD + RichColor.RED,
                )
            time.sleep(1)
        else:
            cprint(
                f"Failed to get data from {url}", style=TextStyle.BOLD + RichColor.RED
            )
            return None

    def _create_url(
        self, product: BybitProduct, resolution: Resolution, date: str | datetime.date
    ) -> str:
    ...
    def _convert_orderbook_data_to_df(self, zipped_data: bytes) -> pl.DataFrame:
        ...
        struct_cols = [
            name for name, dtype in df.schema.items() if isinstance(dtype, pl.Struct)
        ]
        if struct_cols:
