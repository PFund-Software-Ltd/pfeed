...
tDATASET = Literal[
    "DBEQ.BASIC", "GLBX.MDP3", "NDEX.IMPACT", "IFEU.IMPACT", "OPRA.PILLAR", "XNAS.ITCH"
]


# NOTE: 2024-10-26, best free llm for deriving databento dataset and start date is 'mistral'
class DatabentoSource(DataProviderSource):
    def __init__(self):
    ...
    # TODO
    def create_product(
        self, product_basis: str, symbol="", **product_specs
    ) -> BaseProduct:
    ...
    def derive_start_date_using_llm(self, symbol: str) -> str | None:
        ...
        """
        return llm.ask(
            f"{symbol=} current_year={datetime.datetime.now().year}", context=context
        ).strip()
    ...
    def _derive_end_date(self, start_date: str) -> str:
        ...
        return (convert_to_date(start_date) + datetime.timedelta(days=1)).strftime(
            "%Y-%m-%d"
        )
    ...
    def derive_dataset_using_llm(self, symbol: str) -> tDATASET | None:
        ...
        """
        return (
            llm.ask(symbol, context=context).strip().replace('"', "").replace("'", "")
        )
    ...
    def convert_instrument_id_to_raw_symbol(
        self,
        dataset: Dataset | str,
    ...
    def get_child_instruments(
        ...
        if not dataset:
            raise ValueError(
                'Failed to derive dataset, please specify "dataset" or call pe.add_plugin(LlmPlugin(...))() to set up for LLM usage if you haven not done so'
            )
        start_date = start_date or self._derive_start_date(parent_symbol)
        if not start_date:
            raise ValueError(
                'Failed to derive start date, please specify "start_date" or call pe.add_plugin(LlmPlugin(...))() to set up for LLM usage if you haven not done so'
            )
