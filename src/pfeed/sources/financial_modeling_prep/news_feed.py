...
class FinancialModelingPrepNewsFeed(NewsFeed):
    ...
    def _normalize_raw_data_from_FMP_articles(
    ...
    def _download_impl(self, data_model: NewsDataModel) -> dict[str, list[list[dict]]]:
        ...
        datas: dict[str, list[list[dict]]] = asyncio.run(
            self._adownload_impl(data_model)
        )
        for func, data in datas.items():
            if not data:
                self.logger.warning(
                    f"no data downloaded using function {func} for {data_model}"
                )
            else:
                self.logger.debug(f"downloaded using function {func} for {data_model}")
        return datas

    async def _adownload_impl(
        self, data_model: NewsDataModel
        ...
        if self.data_source.plan >= FMPPlan.BASIC:
            ...
            tasks["FMP_articles"] = self.api.aFMP_articles(
        ...
        if self.data_source.plan >= FMPPlan.STARTER:
            if product.is_stock():
                tasks["search_stock_news"] = self.api.asearch_stock_news(
                    symbols=symbols, from_=start_date, to=end_date
                )
            elif product.is_crypto():
                tasks["search_crypto_news"] = self.api.asearch_crypto_news(
                    symbols=symbols, from_=start_date, to=end_date
                )
            elif product.is_fx():
                tasks["search_forex_news"] = self.api.asearch_forex_news(
                    symbols=symbols, from_=start_date, to=end_date
                )
        if self.data_source.plan >= FMPPlan.PREMIUM:
            tasks["search_press_releases"] = self.api.asearch_press_releases(
                symbols=symbols, from_=start_date, to=end_date
            )
