...
if TYPE_CHECKING:
    ...
    from pfeed.sources.pfund.component_data_model import (
        BaseArtifact,
        DataArtifact,
        ModelArtifact,
        SourceArtifact,
        ...
        **io_kwargs: Any,
        ...
        if artifact_type == ArtifactType.data:
            ...
            if self.component_type != ComponentType.model:
                raise ValueError(
                    f"{self.component} must be a model when loading {artifact_type=})"
                )
            try:
                from pfund.components.models.sklearn_model import SklearnModel
            except ImportError:
            ...
            if SklearnModel and isinstance(self.component, SklearnModel):
                ...
                # TEMP
                self.logger.warning(
                    f"Dumped sklearn model {self.component.name} to {file_path}"
                )
                ...
                raise ValueError(
                    f"{self.name} load() is not supported for model: {self.component}"
                )
        # TODO: write .py files
        elif artifact_type == ArtifactType.source:
            pass
            ...
            raise ValueError(
                f"{self.name} load() is not supported for {self.component} ({artifact_type=})"
            )
        return self
