...
class YahooFinanceMarketFeed(StreamingFeedMixin, YahooFinanceMixin, MarketFeed):
    ...
    def _normalize_raw_data(df: pl.LazyFrame) -> pl.LazyFrame:
        ...
        df = df.rename(
            {col: col.replace(" ", "_").lower() for col in df.collect_schema().names()}
        )
    ...
    def _check_yfinance_kwargs(
        ...
        assert "interval" not in yfinance_kwargs, (
            "`interval` duplicates with pfeed's `resolution`, please remove it"
        )
        assert "period" not in yfinance_kwargs, (
            "`period` duplicates with pfeed's `rollback_period`, please remove it"
        )
        assert "start" not in yfinance_kwargs, (
            "`start` duplicates with pfeed's `start_date`, please remove it"
        )
        assert "end" not in yfinance_kwargs, (
            "`end` duplicates with pfeed's `end_date`, please remove it"
        )
    ...
    def download(
        ...
        **product_specs: Any,
        ...
        """
        self._yfinance_kwargs: dict[str, Any] = self._check_yfinance_kwargs(
            yfinance_kwargs
        )
    ...
    def _download_impl(
        self, data_model: YahooFinanceMarketDataModel, data_resolution: Resolution
    ) -> pl.LazyFrame | None:
        # convert pfund's resolution format to yfinance's interval
        timeframe = repr(data_resolution.timeframe)
        etimeframe = self._ADAPTER["timeframe"].get(
            timeframe, timeframe
        )  # external timeframe
        ...
        while num_retries:
            num_retries -= 1
            self.logger.debug(
                f"downloading {product} {data_resolution} from {start_date} to {end_date}"
            )
            ...
            if df is None or df.empty:
                if num_retries:
                    self.logger.info(
                        f"failed to download {symbol} {data_resolution} data, retrying..."
                    )
                    time.sleep(1)
            else:
                ...
                if str(start_date) == self.DAILY_DATA_ROLLBACK_MAX_START_DATE:
                    ...
                    self.logger.debug(
                        f"got actual start date, set start_date={start_date}"
                    )
    ...
    def get_option_chain(
        self, symbol: str, expiration: str, option_type: Literal["CALL", "PUT"]
    ) -> IntoFrame:
