...
class StreamAPI:
    def __init__(self, verbose: bool = False):
        ...
        self._callback: Callable[[RawMessage], Awaitable[None] | None] | None = None
    ...
    def add_channel(
        self,
        data_model: YahooFinanceMarketDataModel,
        data_resolution: Resolution | None = None,
    ) -> ChannelKey:
        resolution: Resolution = (
            data_resolution or data_model.resolution
        )  # data model is using target resolution
        assert resolution.is_tick(), (
            'Only tick data is supported for Yahoo Finance Streaming, please use .stream(resolution="1tick") instead'
        )
        product: BaseProduct = data_model.product
        if (
            product.is_option()
            or product.is_crypto()
            or product.is_future()
            or product.is_index()
        ):
            raise ValueError(
                f"{product.symbol} is not supported for Yahoo Finance Streaming, only stocks, forex, mutual funds, ETFs are supported"
            )
