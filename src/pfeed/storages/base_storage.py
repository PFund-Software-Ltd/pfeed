...
class BaseStorage:
    ...
    def with_io(self, io_config: IOConfig) -> BaseStorage:
        ...
        if self._has_only_one_io_format():
            io_config = io_config.model_copy(
                update={"io_format": self.SUPPORTED_IO_FORMATS[0]}
            )
    ...
    def with_sink(self, sink_config: SinkConfig) -> BaseStorage:
        ...
        if self._has_only_one_sink():
            sink_config = sink_config.model_copy(
                update={"sink": self.SUPPORTED_SINKS[0]}
            )
        ...
        self._sink = Sink(io=self.io, sink_config=sink_config)
        self._is_data_handler_stale = True
        return self
    ...
    def read_metadata(self) -> StorageMetadata:
        ...
        return StorageMetadata(
            ...
            missing_source_paths=cast(
                "list[SourcePath]", self.data_handler.find_missing_source_paths()
            ),
