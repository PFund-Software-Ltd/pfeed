...
class DeltaLakeStorageMixin:
    ...
    def vacuum_delta_table(
        ...
        if files_deleted:
            cprint(
                f"{verb} {len(files_deleted)} files from {table_uri}:",
                style=TextStyle.BOLD + color,
            )
            cprint(
                Panel(
            ...
            cprint(
                f"No files to delete from {table_uri}",
    ...
    def optimize_delta_table(
        ...
        cprint(
            f"Table: {delta_table.table_uri}", style=TextStyle.BOLD + RichColor.GREEN
        )
        ...
        cprint(
            Panel(pretty_output, title="Detailed Metrics", border_style=RichColor.BLUE)
        )
