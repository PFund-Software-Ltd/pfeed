...
class BarMessage(MarketDataMessage, frozen=True):
    ...
    def __post_init__(self):
        if self.is_incremental and self.ts >= self.end_ts:
            raise ValueError(
                f"Timestamp {self.ts} is greater than end timestamp {self.end_ts} before the bar is closed"
            )

        # Validate high is highest
        if not (
            self.high >= self.open and self.high >= self.low and self.high >= self.close
        ):
            raise ValueError(f"High ({self.high}) must be >= open, low, and close")

        # Validate low is lowest
        if not (
            self.low <= self.open and self.low <= self.high and self.low <= self.close
        ):
