...
class ZeroMQDataChannel(StrEnum):
    ...
    def create_market_data_channel(product: BaseProduct, resolution: Resolution) -> str:
        return f"{product.source}.{resolution!r}.{product.name}"
...
class ZeroMQ:
    ...
    def __init__(
        ...
        *,
        ...
        """
        assert any([sender_type, receiver_type]), (
            "Either sender_type or receiver_type must be provided"
        )
        self._name = name
        self._logger = logger
        ...
        self._target_identity: bytes | None = (
            None  # identity of the sender socket to send to, currently only used for ROUTER socket
        )
        ...
        self._msg_decoder = (
            msgpack.Decoder(type=recv_type) if recv_type is not None else None
        )
    ...
    def _assert_socket_initialized(
        self, socket: zmq.Socket, socket_method: SocketMethod
    ):
        ...
        assert self._socket_methods[socket] == socket_method, (
            f"{socket=} registered with socket_method={self._socket_methods[socket]}"
        )
    ...
    def terminate(self, target_identities: list[str] | None = None):
        ...
        for socket in [self._sender, self._receiver]:
            ...
            socket.setsockopt(
                zmq.LINGER, 5000
            )  # wait up to 5 seconds, then close anyway
            socket.close()
    ...
    def send(
        self, channel: str, topic: str, data: Any, target_identity: str | None = None
    ) -> None:
        """
        ...
        try:
            ...
            self._logger.warning(
                f"{self.sender_name} zmq.error.Again, HWM={hwm}, {msg=}"
            )
        except zmq.error.ContextTerminated:
            self._logger.debug(f"{self.sender_name} context terminated")
        except zmq.ZMQError as err:
            self._logger.exception(
                f"{self.sender_name} send() exception (errno={err.errno}):"
            )
        except Exception:
    ...
    def recv(self) -> tuple[str, str, Any, float] | None:
        try:
            ...
            if events:
                ...
                if (
                    self._msg_decoder is None
                    or channel in ZeroMQDataChannel.__members__
                ):
                ...
                return (
                    ...
                    topic.decode(),
                    decoded_data,
                    self._decoder.decode(msg_ts),
                )
            else:
                return None
        except (
            zmq.error.Again
        ):  # no message available, will be raised when using zmq.NOBLOCK
            self._logger.warning(
                f"{self.receiver_name} zmq.error.Again, no message available"
            )
        except zmq.error.ContextTerminated:
            self._logger.debug(f"{self.receiver_name} context terminated")
        except zmq.ZMQError as err:
            self._logger.exception(
                f"{self.receiver_name} recv() exception (errno={err.errno}):"
            )
        except Exception:
