...
class TypeMapper:
    ...
    def __init__(self, custom_mappings: dict[str, pa.DataType] | None = None):
        ...
        self.type_map = {
            ...
            list: pa.list_(
                pa.string()
            ),  # Explicit typing preferred: list[int], list[str], etc.
            dict: pa.map_(
                pa.string(), pa.string()
            ),  # Explicit typing preferred: dict[str, int], etc.
        }

    def python_type_to_pyarrow(
        self, python_type: Any, field_name: str | None = None, value: Any = None
    ) -> pa.DataType:
        """
        ...
        if python_type is list and isinstance(value, list) and value:
            ...
            element_type = self.python_type_to_pyarrow(
                type(first_element), None, first_element
            )
    ...
    def _is_nested_model(self, python_type: Any) -> bool:
        ...
        try:
            ...
            return isinstance(python_type, type) and (
                issubclass(python_type, BaseModel) or issubclass(python_type, Struct)
            )
        except TypeError:
            return False
    ...
    def _convert_model_to_struct(self, model_class: Any) -> pa.DataType:
        ...
        # Use the same inspection logic as the main schema builder
        if issubclass(model_class, BaseModel):
            ...
            for field_name, field_def in model_fields.items():
                ...
                struct_field = pa.field(
                    field_name, pyarrow_type, nullable=not is_required
                )
            ...
            if hasattr(model_class, "__struct_fields__"):
                for field_name in model_class.__struct_fields__:
                    ...
                    pyarrow_type = self.python_type_to_pyarrow(field_type, field_name)
                    struct_field = pa.field(
                        field_name, pyarrow_type, nullable=is_nullable
                    )
                    struct_fields.append(struct_field)
    ...
    def get_field_info(self, model_class: Any) -> dict[str, dict[str, Any]]:
        ...
        if hasattr(model_class, "__struct_fields__"):
            for field_name in model_class.__struct_fields__:
                ...
                default_value = (
                    getattr(model_class, field_name, None)
...
class SchemaBuilder:
    """
    Builder to convert pydantic/msgspec models to pyarrow schema.
    ...
    def model_to_schema(
        ...
        if exclude_fields:
            field_info = {
                k: v for k, v in field_info.items() if k not in exclude_fields
            }
